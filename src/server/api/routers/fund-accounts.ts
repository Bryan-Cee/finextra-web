...
export const fundAccountsRouter = createTRPCRouter({
  getAll: protectedProcedure.query(({ ctx }) => {
    console.log("Fetching all fund accounts for user:", ctx.userId);
  ...
  getFundAccountTransactionsSummary: protectedProcedure.query(
    async ({ ctx }) => {
      const accountTotal = await ctx.prisma.transaction
        .groupBy({
          by: ["accountId"],
          where: {
            userId: ctx.userId,
      ...
      const totalWithoutRecent = accountTotal.map(async (account) => {
        ...
        const _transactionsWithoutRecent =
        ...
        const _totalWithoutRecent = _transactionsWithoutRecent._sum
          .amount as number;
        ...
        return {
          ...
          percentageIncrease:
            ((account?.amount as number) - _totalWithoutRecent) /
            _totalWithoutRecent,
  ...
  getFundSummaryByAccountId: protectedProcedure
    .input(
      z.object({
        id: z.string(),
      })
  ...
  getFundSummary: protectedProcedure.query<FundSummary[]>(({ ctx }) => {
    ...
    LEFT JOIN
      "Transaction" ON "Transaction"."accountId" = "FundAccount".id
    WHERE "FundAccount"."userId" = ${ctx.userId}
    GROUP BY
      "FundAccount".id,
      "FundAccount".title;`;
  }),
